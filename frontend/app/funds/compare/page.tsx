...
export default function FundComparisonPage() {
  ...
  const fundOptions: FundOption[] = funds || []
  const selectedFunds = useMemo(
    () => fundOptions.filter((fund) => selected.includes(fund.id)),
  ...
  return (
    <div className="space-y-8">
      <header className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Comparison</p>
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Multi-fund comparison</h1>
            <p className="text-gray-600">
              Select up to {MAX_SELECTED} funds to benchmark DPI, IRR, and cash flows side-by-side.
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            ...
            {exportMessage && <span className="text-xs text-gray-500">{exportMessage}</span>}
          </div>
        </div>
      </header>
      ...
      <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <p className="text-xs text-gray-500">You can pick up to {MAX_SELECTED} funds.</p>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-gray-900 font-semibold">{fund.name}</span>
                  <input
                    type="checkbox"
                ...
                <p className="text-xs text-gray-500">
                  {[fund.fund_type, fund.vintage_year].filter(Boolean).join(' • ')}
                </p>
                <div className="mt-2 text-xs text-gray-700">
                  ...
                  | IRR:{' '}
                  <span className="font-semibold">
                    {fund.metrics?.irr ? formatPercentage(fund.metrics.irr) : '—'}
                  </span>
                </div>
              </label>
            )
...
function ComparisonResults({ results }: { results: any[] }) {
  if (!results.length) {
    return (
      <DataState
        status="empty"
    ...
    <div className="space-y-6">
        ...
        {results.map((fund) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-gray-500 text-right">
                ...
                <p className="text-lg font-bold text-blue-600">
                  {fund.metrics?.dpi ? Number(fund.metrics.dpi).toFixed(2) : '—'}
                </p>
              </div>
            </div>
            <dl className="mt-4 space-y-2 text-sm">
              ...
              <MetricRow label="Distributions" value={fund.metrics?.total_distributions} />
            </dl>
            <div className="mt-4">
              <p className="text-xs font-semibold uppercase text-gray-500">Cash flow highlights</p>
              <ul className="mt-2 space-y-1 text-xs text-gray-600">
                {fund.cash_flows.slice(-3).map((flow: any, index: number) => (
                  <li key={`${fund.fund_id}-${index}`} className="flex items-center justify-between">
                    <span>{new Date(flow.date).toLocaleDateString()}</span>
                    <span
                      className={cn(
                        'font-semibold',
                        flow.type === 'distribution' ? 'text-green-600' : 'text-red-600'
                      )}
                    >
      ...
      <div className="overflow-x-auto rounded-2xl border border-gray-100 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {results.map((fund) => (
              <tr key={`table-${fund.fund_id}`} className="border-t text-gray-700">
                <td className="px-4 py-3 font-semibold">{fund.fund_name || `Fund ${fund.fund_id}`}</td>
                <td className="px-4 py-3">{fund.metrics?.dpi ? Number(fund.metrics.dpi).toFixed(2) : '—'}</td>
                <td className="px-4 py-3">
                  {fund.metrics?.irr ? formatPercentage(Number(fund.metrics.irr)) : '—'}
                </td>
                <td className="px-4 py-3">
                  {fund.metrics?.pic ? formatCurrency(Number(fund.metrics.pic)) : '—'}
                </td>
                <td className="px-4 py-3">
